...
# This program contains code for training an AI model with the Random Forest Classifier
# The model is designed to predict the height(floor, not_floor) and weight(light, medium, heavy)
# of an object picked up. You can use the trained model by running height-weight-predictor-modelB.py
# More details on how you can do it can be found in the corresponding python file mentioned above.
...
def compute_features(data, data_for_angle, data_for_force):
    ...
    # Filter columns based on body parts and 'Position'
    selected_columns_force_knee = data.loc[:,
                       (data.columns.get_level_values(0).isin(body_parts)) &
                       (data.columns.get_level_values(1) == 'Position')
                       ]

    # Extract the time column
    time_force_knee = data.iloc[3:, 1].reset_index(drop=True).astype(float)
    # Select columns containing the relevant strings (knee)
    # Extract positions for a body part (e.g., knee)
    x_positions_knee = selected_columns_force_knee.loc[:, ('SkeletonConLow:LKNE', 'Position', 'X')].iloc[3:].reset_index(drop=True).astype(float)
    y_positions_knee = selected_columns_force_knee.loc[:, ('SkeletonConLow:LKNE', 'Position', 'Y')].iloc[3:].reset_index(drop=True).astype(float)
    z_positions_knee = selected_columns_force_knee.loc[:, ('SkeletonConLow:LKNE', 'Position', 'Z')].iloc[3:].reset_index(drop=True).astype(float)

    # Calculate accelerations for each axis
    x_acceleration_knee = calculate_acceleration(x_positions_knee, time_force_knee)
    y_acceleration_knee = calculate_acceleration(y_positions_knee, time_force_knee)
    z_acceleration_knee = calculate_acceleration(z_positions_knee, time_force_knee)

    # Combine into total acceleration (magnitude)
    total_acceleration_knee = np.sqrt(x_acceleration_knee**2 + y_acceleration_knee**2 + z_acceleration_knee**2)

    # Estimate forces (assuming body_mass is the effective segment mass for now)
    body_mass = 75
    forces_knee = estimate_force(total_acceleration_knee, body_mass)

    # Compute summary statistics for forces
    features['force_mean'] = np.mean(forces_knee)
    features['force_std'] = np.std(forces_knee)
    features['force_min'] = np.min(forces_knee)
    features['force_max'] = np.max(forces_knee)
...
def prepare_data(file_list, base_dir):
    ...
    for file_name in file_list:
        ...
        features = compute_features(data, data_for_angle, data_for_force)  # Compute summary statistics
...
output_file = 'testset-results-modelB.xlsx'
wb.save(output_file)

# Save the model to pickle file
joblib.dump(multi_output_model, 'height-weight-classification-modelB.pkl')
print("Multi-output model saved as 'height-weight-classification-modelB.pkl'")
